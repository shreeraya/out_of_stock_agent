...
class DemandForecasterAgent(BaseAgent):
    ...
    def forecast_weekly_demand(self, df_demand: pd.DataFrame, promo_context: str = "") -> pd.DataFrame:
        """Loads baseline weekly demand from spreadsheet and applies LLM adjustments if promo context is provided."""
        ...
        # then ask the LLM to provide adjustment factors or a modified weekly schedule.
        ...
        for (sku, dc), group in sku_dc_groups:
            ...
            system_prompt = (
                ...
                "Your role is to analyze baseline weekly demand forecasts and adjust them based on promotional notes or external market events.\n"
                "You must output a JSON object containing the adjusted weekly demand values.\n"
            ...
            user_prompt = (
                ...
                f"Baseline Weekly Forecast:\n"
                f"{json.dumps(baseline_data, indent=2)}\n\n"
                "Please adjust the forecasted weekly demand units based on the market context. If the context does not affect this SKU or DC, keep the baseline.\n"
                "Return the result as a JSON object with an 'adjusted_forecast' list containing elements with 'Week_Start_Date' and 'Adjusted_Demand_Units' (integers)."
            )
            ...
            try:
                ...
                adjusted_map = {item["Week_Start_Date"]: int(item["Adjusted_Demand_Units"]) for item in parsed.get("adjusted_forecast", [])}
                ...
                print(f"[WARNING] Failed to adjust weekly demand for {sku} at {dc} using LLM. Reverting to baseline. Error: {e}")
